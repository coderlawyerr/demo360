...
import 'appointment_calender/model/group_details.model.dart'
    as GroupDetailsModel;
...
class _AppointmentViewState extends State<AppointmentView> {
  ...
  void showAppointmentDialog(BuildContext context, DateTime selectedTime,
      Bilgi service, int serviceIndex, int slotIndex) {
    final provider = Provider.of<AppointmentProvider>(context, listen: false);
    bool isConfirmed = false; // Randevunun onaylandığını izlemek için
    GroupDetailsModel.Uyegruplari?
        selectedGroup; // Seçilen grup ID'sini saklamak için
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (selectedGroup != null)
                            Text('Seçilen Grup: ${selectedGroup?.grupAdi}'),
                                    ...
                                    ConstrainedBox(
                                      constraints: BoxConstraints(
                                        maxHeight: MediaQuery.of(context)
                                                .size
                                                .height *
                                            0.5, // Maksimum yüksekliği belirler
                                      ),
                                      child: ListView.builder(
                                        shrinkWrap: true,
                                        physics:
                                            NeverScrollableScrollPhysics(), // Render hatalarını önler
                                        itemCount: value.misafirList.length,
                                        itemBuilder: (context, index) {
                                          final misafir =
                                              value.misafirList[index];
                                          return Padding(
                                            padding: const EdgeInsets.symmetric(
                                                vertical: 4.0),
                                            child: Text(
                                                'Ad Soyad: ${misafir['adSoyad'] ?? 'Bilinmiyor'}'),
                                          );
                                        },
                                      ),
                                    ),
                          ...
                          Row(
                            ...
                            children: [
                                    ...
                                    showModalBottomSheet(
                                      ...
                                      builder: (context) {
                                        return FractionallySizedBox(
                                          heightFactor:
                                              0.90, // Yüksekliğin 4/3 oranında açılması için
                                          child: Padding(
                                            padding: EdgeInsets.only(
                                            ...
                                            child:
                                                MisafirAdd(), // MisafirAdd sayfası burada modal olarak açılır
                                          ),
                                        );
                                      },
                                    );
                              ...
                              if (isGroup && selectedGroup == null)
                                Consumer<MisafirAddProvider>(
                                  builder: (context, misafirProvider, child) =>
                                      ElevatedButton(
                                    onPressed: misafirProvider
                                            .misafirList.isNotEmpty
                                        ? () {
                                            showTopSnackBar(
                                              context,
                                    ...
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor:
                                          misafirProvider.misafirList.isNotEmpty
                                              ? Colors.grey
                                              : Color(0xFF5664D9),
                                      padding: EdgeInsets.symmetric(
                                          horizontal: 5, vertical: 5),
                    ...
                    Row(
                      ...
                      children: [
                        TextButton(
                          onPressed: () {
                            if (context
                                .read<MisafirAddProvider>()
                                .misafirList
                                .isEmpty) {
                              provider.updateSlotColor(
                                  serviceIndex, slotIndex, Colors.green);
                            } else {
                              provider.updateSlotColor(
                                  serviceIndex, slotIndex, Colors.yellow);
                            }
                            setState(() {
                              isConfirmed = true;
                            });
    ...
    final overlayEntry = OverlayEntry(
      builder: (context) => Positioned(
        top: MediaQuery.of(context).padding.top +
            10, // Status bar yüksekliği + boşluk
        left: 20,
        right: 20,
        child: Material(
